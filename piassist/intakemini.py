...
intakemini = Blueprint('intakemini', __name__)
...
@intakemini.route('/<tenant_name>/minimum/intake', methods=['POST', 'GET'])
@login_required
def primary(tenant_name):
    ...
    if request.method == 'POST':
        ...
        # Go To Next View
        if client_count > 1:
            return render_template('intakemini/client-intake-min.html', case_id=new_case_id, client_number=2, client_count=client_count, defendant_count=defendant_count, medical_providers=medical_providers,tenant_name=tenant_name, auto_insurance_providers=auto_insurance_providers, health_insurance_providers=health_insurance_providers)
        else:
            return render_template('intakemini/defendant-intake-min.html', case_id=new_case_id, defendant_count=defendant_count, defendant_number=1,tenant_name=tenant_name, auto_insurance_providers=auto_insurance_providers)

    return render_template('intakemini/case-intake-min.html', medical_providers=medical_providers, health_insurance_providers=health_insurance_providers,tenant_name=tenant_name, auto_insurance_providers=auto_insurance_providers)

@intakemini.route('/<tenant_name>/minimum/intake/client', methods=['POST', 'GET'])
@login_required
def client(tenant_name):
    ...
    if request.method == 'POST':
        ...
        if client_number < client_count:
            # Next Client
            client_number += 1
            return render_template('intakemini/client-intake-min.html',tenant_name=tenant_name, case_id=case_id, client_number=client_number, client_count=client_count, defendant_count=defendant_count, medical_providers=medical_providers, health_insurance_providers=health_insurance_providers, auto_insurance_providers=auto_insurance_providers)
        else:
            # Defendant
            return render_template('intakemini/defendant-intake-min.html', tenant_name=tenant_name, case_id=case_id, defendant_number=1, defendant_count=defendant_count, auto_insurance_providers=auto_insurance_providers)

    return render_template('intakemini/client-intake-min.html',tenant_name=tenant_name, medical_providers=medical_providers, health_insurance_providers=health_insurance_providers, auto_insurance_providers=auto_insurance_providers)

@intakemini.route('/<tenant_name>/minimum/intake/defendant', methods=['POST', 'GET'])
@login_required
def defendant(tenant_name):
    ...
    if request.method == 'POST':
        ...
        if defendant_number < defendant_count:
            # NEXT DEFENDANT
            defendant_number += 1
            return render_template('intakemini/defendant-intake-min.html',tenant_name=tenant_name, case_id=case_id, defendant_number=defendant_number, defendant_count=defendant_count, auto_insurance_providers=auto_insurance_providers)
        else:
            # FINISH INTAKE & GO TO CASELOG
            return redirect(url_for('casefile.index' , tenant_name=tenant_name))
    return render_template('intakemini/defendant-intake-min.html' , tenant_name=tenant_name)